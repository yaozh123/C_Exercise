...
void log_request(server *serv, connection *con) {
    ...
    if (resp->content_length > -1 && req->method != HTTP_METHOD_HEAD) {
        snprintf(content_len, sizeof(content_len), "%d", resp->content_length); //将resp->content_length参数按照%d格式化成字符串存入content_len中
    } else {
        strcpy(content_len, "-");
    }

    inet_ntop(con->addr.sin_family, &con->addr.sin_addr, host_ip, INET_ADDRSTRLEN);//IP地址转换函数,将“二进制整数” －> “点分十进制”
    date_str(date);// 生成时间戳字符串

    // 日志中需要记录的项目：IP，时间，访问方法，URI，版本，状态，内容长度
    if (serv->use_logfile) {
        fprintf(serv->logfp, "%s - - [%s] \"%s %s %s\" %d %s\n",
                host_ip, date->ptr, req->method_raw, req->uri,
                req->version_raw, con->status_code, content_len);//根据指定的格式，向输出流写入数据
        fflush(serv->logfp);//刷新流 stream 的输出缓冲区
    } else {
        syslog(LOG_ERR, "%s - - [%s] \"%s %s %s\" %d %s",
                host_ip, date->ptr, req->method_raw, req->uri,
                req->version_raw, con->status_code, content_len);//将信息记录至系统日志文件
    }
...
static void log_write(server *serv, const char *type, const char *format, va_list ap) {
    ...
    if (serv->use_logfile) {
        string_append_ch(output, '\n');
        vfprintf(serv->logfp, output->ptr, ap);//使用参数列表发送格式化输出到流serv->logfp中
        fflush(serv->logfp);//fflush()会强迫将缓冲区内的数据写回serv->logfp文件中
    } else {
